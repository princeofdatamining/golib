...
func RegisterContentType(extension, mimetype string) (origin string) {
    origin = mimeDict[extension]
    mimeDict[extension] = mimetype
    return origin
}

var defaultContentType = "application/octet-stream"

// Additionally, specifies the charset(default as UTF-8) for text/* types.
func ContentTypeByExtension2(extension, charset string) (string) {
    if extension == "" {
        return defaultContentType
    }
    ct, ok := mimeDict[extension]
    if !ok {
        return defaultContentType
    }
...
func ContentTypeByExtension(extension string) (ct string) {
    return ContentTypeByExtension2(extension, "")
}
...
func ContentTypeByFilename2(filename, charset string) string {
    dot := strings.LastIndex(filename, ".")
    if dot < 0 || dot+1 >= len(filename) {
        return defaultContentType
    }
    return ContentTypeByExtension2(filename[dot+1:], charset)
}
func ContentTypeByFilename(filename string) string {
    return ContentTypeByFilename2(filename, "")
}
